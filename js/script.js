...
function onLocoScroll(instance) {
  ...
  if (scrollBar) scrollBar.style.width = Math.min((sy / limit) * 100, 100) + '%';
  ...
  document.querySelectorAll('.nav-link').forEach(l => {
    ...
    const section = l.dataset.section || (href ? href.replace('#', '') : '');
    const isPageMatch = href && !href.startsWith('#') && href === currentPage;
    l.classList.toggle('active', (section && section === current) || isPageMatch);
  });
}
